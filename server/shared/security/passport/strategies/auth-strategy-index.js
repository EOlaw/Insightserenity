...
// const Organization = require('../../../../hosted-organizations/organizations/models/model');
...
// CORRECTED: Import the correct HostedOrganization model
const HostedOrganization = require('../../../../hosted-organizations/organizations/models/model');
const TokenService = require('../../../auth/services/token-service');
const config = require('../../../config/config');
const User = require('../../../users/models/user-model');
const { AuthenticationError } = require('../../../utils/app-error');
...
class AuthStrategiesManager {
  ...
  async configureJwtStrategy() {
    const strategy = new JwtStrategy(this.jwtOptions, async (req, payload, done) => {
      try {
        ...
        // CORRECTED: Load user with organization context using correct model
        const user = await User.findById(payload.userId)
          .select('+active +security.lockoutUntil')
          .populate({
            path: 'organization.current',
            select: 'name slug platformConfig subscription status tenantId tenantCode',
            model: 'HostedOrganization'
          })
          .lean();
  ...
  configureSessionSerialization() {
    ...
    passport.deserializeUser(async (sessionData, done) => {
      try {
        // CORRECTED: Specify the correct model for population
        const user = await User.findById(sessionData.id)
          .select('_id email role active firstName lastName organization')
          .populate({
            path: 'organization.current',
            select: 'name slug platformConfig',
            model: 'HostedOrganization'
          })
          .lean();
  ...
  createOrganizationContextMiddleware() {
    return async (req, res, next) => {
      ...
      try {
        // CORRECTED: Ensure organization context is loaded using correct model
        if (!req.user.currentOrganization && req.user.organization?.current) {
          const currentOrg = await HostedOrganization.findById(req.user.organization.current)
            .select('_id name slug platformConfig subscription status tenantId tenantCode')
            .lean();
